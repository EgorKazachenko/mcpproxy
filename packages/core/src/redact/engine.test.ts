...
describe('R13: разрешение пересечений', () => {
  ...
  it('более длинное выигрывает, даже если начинается ПОЗЖЕ короткого', () => {
    ...
    const { text } = redactor.redact('aaaabbbbbbbbbbbb', IN);
    expect(text).toBe(`aa${placeholder('wide')}`);
  });
  ...
  it('повторный скан того же текста даёт тот же результат', () => {
    // `lastIndex` у скомпилированного RE2 — состояние экземпляра, а экземпляр живёт между
    // вызовами: без явного сброса второй вызов начинал бы с середины и пропускал секрет.
    const redactor = createRedactor();
