...
describe('движок не видит одиночного суррогата — он его подменяет', () => {
  it('^.{0,64}$ пропускает одиночный суррогат, матча ДРУГУЮ строку', () => {
    // Замерено: `new RE2('^.{0,64}$', 'u').exec('ab\uD800c')` возвращает `['ab\uFFFDc']` —
    // движок заменяет непарный суррогат на U+FFFD и решает про строку, которой на входе не
    // было. То есть паттерн не может отбить такое значение в принципе: дело не в том,
    // насколько строг автор манифеста, а в том, что проверяемая строка и проверенная — разные.
    //
    // Потребителю этого файла: значение после такой проверки всё ещё непригодно к
    // канонизации, поэтому пригодность обязан проверять тот, кто пишет событие, — паттерн
    // такой гарантии не даёт и дать не может.
    ...
    // Строгий паттерн отбивает подменённую строку — но по составу символов, а не потому,
    // что увидел суррогат: U+FFFD не входит в `[\w./-]`. Гарантии это не даёт.
