 ...
 * Обновляется он только явным решением владельца и только скриптом `update-api-surface.mjs`.
 * Бамп `CONTRACTS_VERSION` при этом обязателен не всегда: `docs/07-contracts.md` объявляет,
 * что добавление необязательного поля версию не двигает, — двигают её удаление, сужение и
 * смена формы. Здесь важно другое: диф снапшота и есть аудируемая запись о том, что шов
 * изменился, поэтому обновить его молча нельзя.
 */
