...
describe('граф зависимостей корневого входа', () => {
  ...
  it('граф деклараций тоже собран — иначе проверка ниже пуста', () => {
    // У `.js`-половины контроль на непустоту есть, у `.d.ts`-половины его не было, а
    // извлечение специфаеров там своё — регулярка, не es-module-lexer. Замер: подменив
    // ту ветку на `return []`, обход посещал ОДИН файл вместо тринадцати и фильтр всё равно
    // давал `[]`, то есть гарантия R3 для деклараций держалась ни на чём.
