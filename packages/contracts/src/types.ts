...
export interface Diagnostic {
   ...
   *
   * Следствие, о котором обязан знать потребитель: указатель **лоссовый**. Ключ с невидимыми
   * символами приезжает без них (`tools.ab` для `a<ESC>[31m<U+202E>b`), поэтому подставлять
   * его обратно в `doc.getIn` нельзя — он для человека и для поиска, а не для навигации.
   * Координаты `line`/`column` для навигации есть и остаются точными.
   */
