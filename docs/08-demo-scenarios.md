...
**Показываем.** `tools/list` через прокси. Три инструмента: `run_tests`, `build_project`,
`analyze_logs`. Никакого `execute_command`, никакого `bash`.
...
**Почему работает.** `entry_hash = SHA256(ts ‖ recipe ‖ argv ‖ cwd ‖ profile ‖ verdict ‖ prev_hash)`.
Изменение любой прошлой записи ломает все последующие хэши.

**За этим стоит.** [Аудит](02-architecture.md#аудит).

**Оговорка.** Tamper-**evident**, а не tamper-proof. Атакующий с правами на файл может
переписать лог целиком и пересчитать цепочку. Защита от этого — публикация Merkle-корня
