...
# Проба четвёртого круга — снятие последнего `ASSUMED` перед реализацией

## П10. Байты тела HTTPS под `tlsTerminate` — ЕСТЬ, И РОВНО ТЕ ЖЕ

Task 7 плана начинался с условия: «если проба покажет, что байт нет, — остановиться и
вернуться к владельцу». Показала, что есть.

```
POST http 1234b              exit=0 out="405" err=""
POST https 1234b             exit=0 out="405" err=""

--- filterRequest увидел ---
{"url":"http://example.com/","method":"POST","bodyBytes":1234,"hasBody":true}
{"url":"https://example.com/","method":"POST","bodyBytes":1234,"hasBody":true}
```

**Что доказано.** Под `tlsTerminate` колбэк получает тело HTTPS-запроса целиком и считает
**точное** число байт — 1234 из отправленных 1234, столько же, сколько на plain HTTP.
Цифра «отправлено 1.2 KB» из S5 снимается с HTTPS, а не только с HTTP, и D12 держится.

**Чего проба не покрывает:** чанкованное тело не измерялось отдельно; байты **ответа** колбэк
не даёт ни на одном транспорте.

Остались непокрытыми два `ASSUMED` из §8 плана: поведение SOCKS-пути под `updateConfig`
(нога `nc` в П9 неинформативна) и накладные расходы стадий — последнее снято уже не пробой,
а измерением в `events.test.ts`: p95 суммы `build_env` и `build_profile` по серии из ста
прогонов утверждается порогом в 5 мс при бюджете в 50 мс.
