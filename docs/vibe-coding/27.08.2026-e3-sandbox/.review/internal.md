...
файл описывает и то, что было отревьюено, и то, что с этим сделано).

**Каждая посылка проверена против кода до применения фикса.** Все тринадцать находок
подтвердились; ни одна не отклонена.

- **Critical, политика на аварийном пути** — принят. `applyNetwork(base, IDLE_NETWORK)` и
  drain переехали в `finally`, drain вместе со снятием (иначе нарушения после падения тела
  атрибутировались бы следующему вызову). Тест на аварийном пути живёт в
  `modes/none.test.ts`, а не в `seatbelt`: в `seatbelt` команду запускает обёртка `bash -c`,
  и несуществующий бинарь даёт код 127, а не отказ `spawn`, — то есть в seatbelt эту ветку
  воспроизвести нечем. Мутация «вернуть снятие на успешный путь» краснеет.

- **Major, дедлок ждущих при отравлении** — принят, с уточнением реализации. Выбран второй
  из предложенных путей: семафор освобождается, а отказывает флаг. Каждый ждущий
  просыпается, видит отравление, пропускает очередь дальше и бросает. Требование R52 «ни
  один следующий вызов не идёт» исполняется, а «MCP-запрос повис навсегда без записи в
  аудит» не наступает. Комментарий класса переписан: он утверждал то, что было неправдой для
  уже вставших в очередь.

- **Major, событие `spawn` после выхода процесса** — принят. В `ProcessLimits` добавлен
  `onSpawn`, зовущийся синхронно по возврату из `spawn()`; тело обёрнуто в `try/catch`,
  который эмитит стадию и на провале запуска. `durationUs` стадии стал временем самого
  вызова `spawn`, а не жизни ребёнка: порядок в `stageOrder` заморожен, а время ребёнка
  выводится из меток `spawn`→`complete` у E4. Тест на порядок — в `modes/seatbelt.test.ts`,
  тест на событие при провале запуска — в `modes/none.test.ts` по той же причине, что выше.

- **Major, `unrecognized` и `suppressed` выброшены без следа** — принят. `ExecOutcome`
  получил `unrecognizedLines` и `suppressedLines`. Тест утверждает `suppressedLines > 0` на
  живом вызове (шум есть всегда, П1) и `unrecognizedLines === 0` — ненуль означал бы, что
  грамматика отстала от системы.

- **Major, `attributionMismatches` слеп к отсутствию ключа** — принят, разведением на два
  счётчика, как и предлагалось альтернативой. Один счётчик на оба случая читался бы как
  тревога на каждом втором вызове: отсутствие — рутина, чужой ключ — сигнал.

- **Major, полный URL с query в цепочку аудита** — принят. Добавлен `redactUrlForTarget` той
  же формы, что вендорский `redactUrlForViolation`. Инжектируемого шва не заводили: сведение
  безусловно и не зависит от правил E6, а лишний слот пришлось бы кому-то заполнять.
  Мутация «не сводить URL» краснеет на четырёх утверждениях.

- **Major, неограниченный `telemetry`** — принят. Поле и `telemetrySnapshot()` удалены
  целиком: те же данные уже доезжают потребителю как `SandboxViolation`.

- **Major, исключение потребителя роняет демон** — принят. `dispatch` обёрнут в `try/catch`
  со счётчиком `consumerFailures`. Мутация подтвердила диагноз буквально: без обёртки прогон
  даёт `Unhandled Errors: дефектный потребитель`.

- **Minor, нет слушателя `error` на потоках** — принят. `EPIPE` на убийстве группы —
  штатное событие каждого таймаута, то есть падение демона стояло ровно на том пути, ради
  которого таймаут заведён.

- **Minor, расхождение `termination` и `truncated`** — принят. Исход считается по тем же
  флагам, что и `truncated`. Первая мутация здесь **выжила**: фикстуры, где два основания
  расходятся, не было. Добавлена — 70 байт при потолке 64 и редакции до десяти, — после чего
  мутация краснеет.

- **Minor, R15 и разрешённый сырой TCP** — принят в редакции «записать границу»: строка в
  `docs/10-honest-limitations.md` теперь говорит, что для сырого TCP **записи нет вовсе**, а
  не что она с нулём байт. Хук на SOCKS-путь не заводим: он у вендора не выставлен, и это
  само по себе ответ.

- **Minor, `countBody` держит медленное тело** — принят. Чтение ограничено и по времени
  (`BODY_SAMPLE_MS`), а не только по байтам.

- **Minor, `disposed` навсегда запирает процесс** — принят. Флаг «эта песочница мертва»
  переехал в экземпляр; процессного больше нет. R50 требует отказа у **освобождённого
  экземпляра**, а не вечной непригодности процесса, и `reset()` чистит
  `initializationPromise`, поэтому переподъём безопасен. Тест утверждает обе половины:
  освобождённая бросает, свежая работает.
