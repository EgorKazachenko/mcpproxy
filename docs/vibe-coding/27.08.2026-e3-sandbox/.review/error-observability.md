...
**Все семь находок приняты, ни одна не отклонена.** Каждая посылка проверена против кода и
против исходника вендора до применения фикса.

- **Major, errno-слепой `groupAlive`** — принят. Решение по коду ошибки вынесено в чистую
  `isGroupGone`, потому что иначе ветка непроверяема: чтобы получить `EPERM` от
  `kill(-pid, 0)`, нужна чужая группа процессов, а тест, который её заводит, зависит от прав
  машины больше, чем от нашего кода. Литералом оба кода подаются тривиально. Мутация
  «схлопнуть errno» краснеет на двух утверждениях.

- **Major, нет `try/finally` после `spawn`** — принят. Убийство группы и подтверждение
  пустоты вынесены за `try`, то есть исполняются на любом пути. Отказ несёт `groupDrained` в
  контексте `ExecError`, и `withNetworkPolicy` отравляется на нём так же, как на штатном
  `outcome.groupDrained === false`. Диагноз воспроизведён буквально: до правки бросок
  аудит-стока на стадии `spawn` оставлял живую группу и не порождал `ExecOutcome` вовсе.

- **Major, `catch` уносил запись, а не только решение** — принят, и это худшая из семи по
  последствиям. Обоснование «ошибка даёт allow, а не deny» действительно отвечало на другой
  вопрос. Инвариант «сбой счётчика уносит байты, а не запись» вынесен в чистую
  `telemetryRecord`: ветка недостижима чёрным ящиком — вендор отдаёт свежую ветку tee, —
  поэтому подаётся литералом. Мутация «выбросить запись вместе с байтами» краснеет.

- **Major, поздние нарушения ретируются без счётчика** — принят. Добавлен
  `lateUnattributed`: счётчик уровня синглтона, снимаемый в начале следующего вызова.
  **Ветка осталась непокрытой тестом, и это записано, а не умолчано:** воспроизвести
  «нарушение приехало, когда активного вызова нет» можно только гонкой с недостаточным
  drain-окном, и тест, который её подстраивает, проверял бы задержку машины, а не наш код.
  Мутация «не считать» остаётся зелёной. Класс тот же, что у ветки отравления R52, и
  честнее объявить его, чем построить утверждение, которое ничего не различает.

- **Major, `SrtManagerError` без дискриминанта и вне поверхности** — принят целиком, включая
  предложенный список кодов. Введён `ExecError` с `code` и `context`; все восемь точек
  отказа переведены на него; класс поднят в поверхность и в снапшот. Вынесен в отдельный
  `errors.ts`, иначе получался цикл `netpolicy → sandbox → modes → netpolicy`.
  `poisoned` и `group-not-drained` разведены, как и предлагалось.

- **Minor, слушатель `error` не складывал потерю в `truncated`** — принят. Диагноз точен:
  «полный вывод» объявлялся там, где байты пропали, а `truncated` — ровно то поле, на
  котором висят R19 и R20.

- **Minor, отказы без идентификатора** — принят. `ExecError.context` несёт `recipeName`,
  `commandId`, `pid` и `groupDrained`; тексты отравления называют вызов и лидера группы.

**Отдельно — про замечание о рабочем дереве.** Наблюдение верное и полезное:
`detached: true → false` в тот момент действительно лежал в дереве, это была мутационная
проба параллельной размерности test-quality. Она восстановила файл сама, дерево проверено
чистым перед коммитом, и в `660a4fe` мутация не попала.
