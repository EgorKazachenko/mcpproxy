...
9а. `ValidatedValues` и `ResolvedValues` — две **различимые** формы одной карты, брендированные
   уникальными символами. Обе стирается в `ReadonlyMap<string, ParamValue>` в рантайме, цены
   нет. Нужны потому, что на шве фасада обе карты существуют одновременно, а без бренда они
...
9б. `DENIALS_MAX = 32` — потолок на длину списка отказов (R30а). Значение выбрано так, чтобы
...
9. **Собственная перепроверка инвариантов, а не доверие загрузке.** `prepareRecipe` принимает
   ...
   **относительный `root` не выходит за `manifestDir`**, отсутствие слота `{}` в `exec` и в
   `cwd`. Это R22 в исполнении — тест на чужой инвариант проверял бы чужой код, а не поведение
   нашего на нарушенном входе.

   Второе правило нельзя опускать, взяв только первое: они стоят рядом в
   `packages/contracts/src/validate/refine.ts:212` и `:223` как две половины одной проверки, и
...
| R20 | Задача 6, шаг 5 |
| R21 | Задача 6, шаг 4 |
| R22 | Задача 3, шаг 9 (слот в `exec`/`cwd`, `root: '/'`, относительный root) + задача 6, шаги 1 и 7 |
...
| R30 | Задача 2, шаг 9 (константа) + задача 4, шаги 6, 7 и 10 (применение во всех трёх строковых типах) |
| R30а | Задача 2, шаг 9б (константа) + задача 4, шаг 2а (применение) |
| R31 | Задача 7, шаг 4 |
