...
1. В `packages/core/package.json` добавить `"test": "tsc -b && vitest run"` в `scripts`,
   а в `devDependencies` — `"vitest": "^3"` и `"es-module-lexer": "^1"`.
   `es-module-lexer` объявляется явно, хотя сегодня разрешился бы и без этого: при
   `nodeLinker: node-modules` он захойстен в корень из-за `packages/contracts`, то есть
   гарантия R1 держалась бы на чужой devDependency соседнего пакета.
...
9. `VALUE_MAX_CODE_POINTS` — абсолютный потолок длины строкового значения (R30), независимый
   ...
   и записи аудита. Класс уже описан в `docs/07-contracts.md:218`, и вывод там тот же — потолок.
...
`string` снова получает `| null` или поле `schema`. Форма — по образцу
`packages/contracts/src/domain.test.ts:27-31`. Без него замкнутость формы существует только
...
4. **Гейт канонизируемости и длины — на каждое строковое значение** (R28, R30), до проверок
   типа-специфичных ограничений: `isCanonicalizable` не прошло → `not-canonicalizable`;
   `[...value].length > VALUE_MAX_CODE_POINTS` → `value-oversized`.
   Это и есть закрытие Ф12: полагаться на паттерн автора манифеста нельзя, потому что
   `^.{0,64}$` — законный паттерн, пропускающий одиночный суррогат. Гейт стоит **до**
   `matcher.test`, чтобы вердикт не зависел от того, насколько строг автор.
...
export type ResolvePathsResult =
  | { ok: true; argvValues: ReadonlyMap<string, ParamValue>; hashValues: ReadonlyMap<string, ParamValue> }
  | { ok: false; denials: readonly [Denial, ...Denial[]] };
...
2. **Лексическая предпроверка** (R15а): собрать `resolve(param.root, value)` и прогнать через
   предикат §7 **до** обращения к файловой системе. Не прошло → `path-escapes-root`.
   Это сужает оракул: без неё явный обход `../../.ssh/id_rsa` отвергается по `ENOENT`, то есть
   код отказа сообщает, существует ли произвольный путь на диске.
3. `realpathSync` над `param.root` (Ф4). Не резолвится → `path-unusable`.
...
5. Тот же предикат §7 — теперь над результатом `realpath` (R13). Не прошло →
   `path-escapes-root`. Шаг 2 его не заменяет: симлинк ловится только здесь.
6. **Две карты на выходе, а не одна** (R17). `argvValues` несёт путь ровно в том виде, в
   каком его вернул `realpath`; `hashValues` — его же в NFC. Причина в том, что задачи разные:
   argv обязан открыть тот файл, существование которого проверил шаг 4, а `argsHash` обязан
   не зависеть от формы записи. На macOS обе формы открывают один файл, на файловой системе
   с байтовыми именами — разные, и нормализация после проверки существования дала бы argv,
   указывающий в никуда.
7. Значения не-`path` параметров попадают в обе карты без изменений.
...
Пятая, на R17: файл создаётся под NFD-именем, запрашивается под NFC. `argvValues` обязан
нести форму из `realpath`, `hashValues` — NFC, и `argsHash` от `hashValues` обязан совпасть с
`argsHash` при запросе под NFD. Одна карта вместо двух → одно из трёх утверждений краснеет.
...
5. Остальные типы: каждый элемент шаблона — отдельный элемент результата; элемент, содержащий
   ...
   в карте уже лежит резолвнутый путь: задача 5 заменила значение на месте, и источника, из
   которого можно взять сырую строку, здесь нет.
...
3. Отказы стадии передаются наружу **все**, непустым кортежем. Задача 4 намеренно собирает
   ...
   оказаться пустой.
4. **Успех отдаёт `params`** — проверенные значения в NFC-форме, то есть `hashValues` из
   задачи 5, уже как обычный объект (`Object.fromEntries`), чтобы шов E4 не имел развилки
   (R31). Без этого `argsHash` считать не из чего: контракт требует значения **после**
   валидации и резолва (`packages/contracts/src/audit/args.ts:9`), и без них нормализация R17
   бессмысленна, а `{file: './logs/a.log'}` и `{file: '/abs/logs/a.log'}` перестают быть одним
   вызовом.
...
6. `cwd` присутствует, если стадия `resolve_paths` была достигнута, в том числе при отказе на
   ...
   отказавший здесь, свой `cwd` уже знает. При отказе на `validate` — `null`. Спрятав его, мы
   отняли бы у S4 предмет демонстрации: границу confinement показывают относительно рабочего
   каталога.
...
На R31: `argsHash(prepared.recipeName, result.params)` не бросает и **совпадает** для двух
вызовов, запросивших один файл в NFC- и NFD-форме. Без `params` в результате первое
утверждение не компилируется, без NFC из задачи 5 — краснеет второе.

На шаг 6: отказ на `resolve_paths` несёт `cwd !== null`, отказ на `validate` несёт
`cwd === null`. Оба утверждения нужны: одно зелено при константе.

Отдельно, уровня типа: ветка `ok: false` не имеет поля `argv` — выражение перестаёт
компилироваться при его появлении, по образцу `packages/contracts/src/domain.test.ts:27-31`.
...
6. **Двусторонняя перепись кодов отказа.** Таблица «код ↔ вектор, который его производит»
   ...
   `packages/contracts/src/validate/refine.test.ts:493` и по тому же доводу, по которому
...
4. Позитивный контроль по образцу `packages/contracts/src/deps.test.ts:89`: утверждение, что
...
(`packages/contracts/src/deps.test.ts:62`).
...
| R15а | Задача 5, шаг 2 — лексическая предпроверка до файловой системы |
| R16 | Задача 5, шаг 4 — `ENOENT` → `path-not-found` |
| R17 | Задача 5, шаг 6 — две карты, argv без нормализации |
| R18 | Задача 3, шаг 8 (единственный вычислитель) + задача 5, шаг 8 (здесь не считается) |
...
| R28 | Задача 2, шаг 8 (предикат) + задача 4, шаг 4 (гейт на каждое строковое значение) |
