...
**F6. Закрыт замером, и не в мою пользу.** Вопрос был, нужен ли `entryFileNames` поверх
`format: 'cjs'`. Разведка утверждала, что не нужен; я рассуждал, что нужен, потому что
расширение выводится из поля `"type": "module"`. Проверено сборкой со снятым пином на
electron-vite 5.0.0 и Electron 43: preload выходит `index.cjs` и без него. **Права была
разведка.** Пин оставлен по другой причине и она записана в конфиге: вывод расширения —
деталь реализации тулчейна, и пин делает гарантию явной, а не зависящей от неё.
...
отсутствующего ключа. `ApprovalRequest` того же поля **не** получает — см. шаг 2.
...
1. Добавить поле и дописать строку `build_argv` в таблицу «стадия → какие поля впервые
   ...
   бампом**, и после этой задачи утверждение станет ложным — правится тем же движением, чтобы
   в диффе, который служит записью об изменении шва, не осталось ложного утверждения.
...
2. **`ApprovalRequest` в этом ране не трогается.** Первая редакция добавляла поле и туда ради
   ...
   подтверждения планируется вместе с правилом. Поэтому утверждение
   `packages/contracts/src/approval.ts:6` — «объявлены целиком в E0, а не дорисованы в E5/E7» —
   остаётся истинным, и файл не трогается вовсе.
