...
`electron-vite@5`, `electron-builder@^26.15.6`, Vite `7.3.6`, Electron 43, React 19,
...
| `AuditEvent` | тесты `otlp.test.ts`, `chain.test.ts`, `approval.test.ts` внутри пакета | строят фикстуры события | это тесты потребителей, а не моки |
...
| `mandatory-deny`, `denied` | `danger` | красный, исход `blocked` | единственная пара, где отбитая попытка красная |
...
### Task 1 — контракт: параметры вызова и происхождение argv

Реализует `R61`, `R62`, `R63`, `R64`.
...
**Interfaces.** Три необязательных поля, добавляемых к существующим формам.
...
`params` появляется на стадии `received`, `argvFromParams` — на `build_argv`, рядом с самим
`argv`. Оба необязательны и подчиняются правилу отсутствующего ключа.
...
1. Добавить поля и дописать их в таблицу «стадия → какие поля впервые появляются» в JSDoc
   `AuditEvent`. Таблица — часть контракта для E6 и E4, и поле без строки в ней означает, что
   производитель события не знает, когда его писать.
2. Добавить `argvFromParams` в `ApprovalRequest` и объяснить в JSDoc, зачем: правило выбора
   токена для набора обязано быть вычислимым из того, что окно получило.
3. Обновить снапшот публичной поверхности **явным скриптом**, а не переменной окружения:
...
4. Дописать `docs/07-contracts.md` там, где перечислены поля события.
5. Коммитом отдельно от всего остального: правка контракта требует ревизии зависимых веток.

**Falsification:** утверждение — `expect(Object.hasOwn(receivedEvent, 'argvFromParams')).toBe(false)`
на событии стадии `received` вместе с `expect(buildArgvEvent.argvFromParams).toEqual([3])` на
событии `build_argv`. Написать поле уже на `received` → первое утверждение расходится, и это
ловит ровно ту ошибку, из-за которой UI показал бы происхождение аргументов у вызова, ещё не
собравшего команду. Второе утверждение — `expect(toOtlp(eventWithParams).attributes.map((a) => a.key)).not.toContain('params')`:
спан по контракту сводка, а не полная запись, и новое поле не имеет права протечь туда без
записи в реестре атрибутов. Тесты в Node.

**Проверка:** `yarn workspace @mcpproxy/contracts test`, затем `yarn build` из корня.

**Коммит:** «E7: контракт получает параметры вызова и происхождение argv».
...
export const outcomeLabel: Readonly<Record<CallOutcome, string>>;
```
...
3. Добавить `CallOutcome` и `outcomeLabel` по всем шести значениям: `blocked` → «Отбито»,
   `passed` → «Прошло», `denied` → «Отказано», `awaiting` → «Ждёт подтверждения»,
   `clean` → «Выполнено», `running` → «Выполняется». Шестое обязательно: проигрыватель отдаёт
   события по одному, и половина вызовов на экране всегда незакончена. Союз объявляется
   здесь, потому что `desktop` зависит от `design`, а не наоборот.
4. Завести пакету скрипт `test` вида `tsc -b && vitest run` и `vitest` в `devDependencies` —
   сегодня скрипта нет, и первый же тест пакета не запустился бы.
5. `README.md`: заголовок «Четыре роли, а не три» неверен, и таблица под ним тоже.
...
из первого правила. Третье — `expect(Object.keys(outcomeLabel)).toHaveLength(6)`: пять
значений заставили бы рендерер чеканить шестую подпись у себя. Тесты в Node.
...
1. **`@mcpproxy/contracts` и `@mcpproxy/design` собираются внутрь main и preload, а не
   ...
   тогда не идёт по симлинку `workspace:*`, и класс багов «electron-builder плюс yarn
   workspaces» исчезает по построению. Упаковка — предмет E9, но выбор бандлинга это конфиг
   этой задачи, и E9 его дёшево не переиграет.
...
4. Разделить tsconfig: main и preload остаются на `lib: ["ES2023"]`; рендерер добавляет `DOM`,
   `DOM.Iterable`, `"jsx": "react-jsx"` и объявление модуля для `@mcpproxy/design/css` — при
   `moduleResolution: NodeNext` спецификатор CSS иначе не резолвится. Ссылки на `../contracts`
   и `../design` остаются в том под-конфиге, который компилирует TS, иначе граф проекта рвётся.
5. `vitest.config.ts` заводит два проекта: юниты и смоук, причём смоук **исключён** из
   обычного `vitest run`, а не только выделен в проект. Иначе каждая «Проверка» начиная с
   Task 8 запускала бы Electron.
...
2. `resolveBundlePath` резолвит запрошенный путь, берёт `realpath` и отдаёт `null` всему, что
   не под корнем сборки; обработчик на `null` отвечает 404. Стандартная схема нормализует
...
5. Ветка режима — по `NODE_ENV`, не по `app.isPackaged`: собранное приложение под смоуком
...
7. Запрет навигации и открытия окон вешается на `app.on('web-contents-created')` — единую
...
export type UiRequest = { readonly kind: 'player-command'; readonly command: PlayerCommand };
...
`UiRequest` в этом ране с одним вариантом, но объявлен размеченным union: ран 2 добавит
вердикт апрува и экспорт лога, и форма не поменяется.
...
export function guarded<T>(
  run: (payload: unknown) => Result<T> | Promise<Result<T>>,
  allowedOrigins: ReadonlySet<string>,
...
3. `parsePlayerCommand` ограничивает скорость: конечное число в закрытом диапазоне.
...
Третье — `expect(parsePlayerCommand({ kind: 'play', speed: Infinity }).ok).toBe(false)`.
...
5. Фикстура покрывает сценарии S1–S9. Формы полей берутся из
   ...
   **отсутствует**, а не приезжает пустым массивом. Хотя бы одно событие несёт старую ревизию
...
6. **Генератор фикстур считает цепочку хэшей.** Поле `chain.self` обязано
   ...
   входа `./audit`, его вывод коммитится, а тест утверждает, что закоммиченная фикстура
   проверяется: иначе одна поздняя правка руками ломает демо молча.
...
что бейдж вердикта глушится, когда нарушение прошло насквозь. Одним полем это не выражается:
вызов при этом остаётся разрешённым, и новость не в вердикте. `CallOutcome` импортируется из
дизайн-системы, где Task 2 объявляет его вместе с подписями: объявить тот же союз здесь
второй раз значило бы завести два независимых источника одного значения.
...
13. **Вся экранная проза живёт в `strings.ts`**, и это обеспечивается структурно, как запрет
...
Пятое — `expect(cyrillicOutsideStrings(rendererSources)).toEqual([])`; захардкодить русскую
...
1. Тест пишется как vitest-тест, импортирующий пусковой модуль Electron из `playwright`, а не
   ...
   обычным прогоном. Проект смоука исключён из обычного прогона конфигом из Task 3.
2. **`R2` проверяется на общей точке всех web contents.** Обработчик `web-contents-created` из
   ...
   четырьмя флагами, а несовпадение валит запуск. Это и есть буквальное «читает фактические
   настройки созданного окна» из спеки.
...
5. Отсутствие `node:crypto`, `ajv` и `re2` проверяется обходом графа импортов по **эмиту
   `tsc` для проекта рендерера**: там `import type` уже стёрт, а относительные импорты ещё не
   схлопнуты. Ни исходники, ни готовый бандл на этот вопрос не отвечают — лексер JS не
   отличает импорт типа от значимого импорта, а бандлер к моменту сборки встраивает случайно
   значимый импорт внутрь, не оставив специфаера.
...
**Falsification:** утверждение — `expect(prefsSeenAtCreation.every((p) => p.sandbox === true)).toBe(true)`,
...
**Проверка:** `yarn workspace @mcpproxy/desktop test --project smoke`.
...
4. Дописать в спеку таблицу покрытия по требованиям рана 1. Требования рана 2 перечислены
...
| `R5` | Task 5, `Interfaces` — чтение `senderFrame` первым оператором до любого `await` |
...
| `R17` | Task 7, шаг 7 — секции вызова, причины отказа, команды, стадий и редакции |
...
| `R49` | Task 7, шаг 13 — AST-страж на кириллицу вне `strings.ts` плюс сверка фрагментов с макетом |
...
| `R61` | Task 1, `Interfaces` — `params` на стадии `received` |
...
| `R64` | Task 1, шаги 3 и 5 — снапшот обновляется явным скриптом, правка идёт отдельным коммитом |

Отложено в ран 2 решением владельца от 2026-08-28, с готовым и прошедшим дизайн-ревью макетом
в качестве спецификации: `R14` (проверка цепочки в main — без вкладки аудита у неё нет
потребителя), `R23`–`R26` (панель нарушений — `R23`, `R24`, `R25`, `R26`; policy viewer — `R27`, `R28`, `R29`,
`R30`, `R31`; вкладка аудита — `R32`, `R33`, `R34`, `R35`; модалка расхождения lock — `R36`,
`R37`, `R38`; окно подтверждения — `R39`, `R40`, `R41`, `R42`, `R43`, `R44`; инбокс апрувов —
`R57`; соответствие ширины гранта — `R59`. Перечислены поимённо, а не диапазонами: диапазон
читается как покрытие, а проверить его нечем.
