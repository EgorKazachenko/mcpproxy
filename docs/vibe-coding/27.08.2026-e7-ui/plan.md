...
Индексы этих двух свойств не имеют: числа, без глубины, без суррогатов, без секретов.
...
1. Добавить поле и дописать строку `build_argv` в таблицу «стадия → какие поля впервые
   ...
   опционального поля версию не двигает, и требовать бампа здесь не нужно.
2. Добавить `argvFromParams` в `ApprovalRequest` и объяснить в JSDoc, зачем: правило выбора
   ...
   Там же поправить вводный JSDoc файла: `packages/contracts/src/approval.ts:5` утверждает,
   ...
   об изменении шва.
3. **Согласовать с E6.** Событие пишет он, и поле, которого никто не пишет, — это заморозка
   ради ничего. `WORK.md` требует ревизии зависимых веток при правке контрактов; ветка
   `v2/e6-audit` жива, и обязанность писать `argvFromParams` на `build_argv` доносится до неё
   этой задачей, а не подразумевается.
...
Третье — `expect(() => chainHash(eventWithField, null)).not.toThrow()`: поле обязано быть
канонизируемым, и это утверждение — то, чего не хватало исходной редакции, добавлявшей в
...
`createWindow` не только конструирует окно, но и **грузит в него содержимое**: в продакшне —
`APP_ORIGIN`, в разработке — адрес dev-сервера. Без этого шага приложение открывает пустое
...
4. Разделить tsconfig, **и переписать `packages/desktop/tsconfig.json` в решение-ссылку** на
   ...
   **`src/shared/` принадлежит проекту main**, а рендерер ссылается на него `references`, а не
   включает файлы вторым `include`: при `composite: true` одни и те же файлы в двух проектах
   дают конфликт эмита. Ссылки на `../contracts` и `../design` живут в обоих под-проектах.
...
3. `parsePlayerCommand` — **разбор написан руками, а не схемой**, и это осознанное отклонение
   ...
   Скорость ограничивается конечным числом в закрытом диапазоне.
...
Пятое — `expect(stageDetail(buildArgvEvent)).toBe(STRINGS.stage.buildArgv({ count: 4, substituted: 1 }))`
...
Шестое — `expect(callLine(mandatoryDenyCall).verdictMuted).toBe(true)` при
...
Седьмое — `expect(cyrillicOutsideStrings(rendererSources)).toEqual([])`; захардкодить русскую
...
2. **`R2` проверяется на общей точке всех web contents.** Обработчик `web-contents-created` из
   ...
   спеки.
   **Проверка включается тем же флагом сборки, что и контракт состояний, и в отгружаемой
   сборке отсутствует.** Валить запуск в проде она не имеет права: тот же обработчик
   срабатывает и на внутренних web contents вроде инструментов разработчика, чьи настройки
   заданных флагов не несут, — самоуничтожение приложения на первом открытии дев-тулов было бы
   дороже дефекта, который оно ловит. Под флагом наблюдения обработчик копит увиденные
   настройки, а утверждает про них смоук. Само API помечено `ASSUMED` в F9.
...
5. Отсутствие `node:crypto`, `ajv` и `re2` проверяется обходом графа импортов по эмиту `tsc`
   ...
   типов сохраняются), ни `build` (это бандлер) его не дают. Смоук вызывает `tsc -b` по
   конфигу рендерера в отдельный каталог и обходит результат. Ни исходники, ни готовый бандл
   на вопрос не отвечают: лексер JS не отличает импорт типа от значимого, а бандлер к моменту
   сборки встраивает случайно значимый импорт внутрь, не оставив специфаера.
