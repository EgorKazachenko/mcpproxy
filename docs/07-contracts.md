...
| `invariant` | Проверка, которую схема выразить не может: confinement, слоты, форма `exec[0]`, потолок `env` |
...
`message` безопасен для отрисовки: всё, что интерполируется в него из недоверенного
манифеста, проходит через `sanitizeDescription`. Без этого сообщение RE2, эхоящее фрагмент
паттерна дословно, донесло бы до глаз человека и до терминала bidi-override и ANSI-escape.
