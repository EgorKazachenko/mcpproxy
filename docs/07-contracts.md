...
**Длительность ограничена и по форме, и по значению.** Схема даёт `Duration` девять цифр —
этого хватает, чтобы значение осталось безопасным целым. Но `999999999h` — законные девять
цифр и 3.6·10¹² мс, а выше максимума таймера платформы (`2^31 − 1` мс) Node молча клампит
таймаут к 1 мс: манифест, просящий «почти никогда не прерывать», получил бы прерывание
немедленно. Поэтому значение проверяется отдельно от формы.
