...
- Границы `number` обязаны быть **выполнимыми**: `min > max` — ошибка загрузки, и при
  `integer: true` — тоже отсутствие целых на отрезке (`min: 1.2, max: 1.8`). Рецепт, чьи
  границы не проходит ни одно значение, отвергал 100 % вызовов и раньше — но молча, на каждом
  значении и без диагностики; теперь он не грузится.

  **Это ужесточение действует на файл, который у установки уже есть**, и отказ `parseManifest`
  всеманифестный: установка с одним таким рецептом теряет **все** инструменты, а не один.
  Тира «предупреждение» у загрузки нет и завести его нельзя — `Diagnostic` не имеет поля
  `severity`, а поверхность заморожена. Обновление обязано нести это в релиз-нотах как
  ломающее изменение.
