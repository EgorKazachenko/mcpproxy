...
| **`read.allow` widens access, it does not narrow it** | A recipe's `read.allow: ["."]` does not confine reads to the recipe directory: the vendor's read model is permissive by default, and the allowlist only adds paths. The only thing that actually holds is an explicit `read.deny`, so a manifest that wants credential files closed has to name them one by one | measured by the E8 corpus: class A10 came back 5 of 8 achieved under `seatbelt` (`~/.netrc`, `~/.npmrc`, `~/.docker`, `~/.kube`, the keychain), while the three paths named in `read.deny` were blocked |
